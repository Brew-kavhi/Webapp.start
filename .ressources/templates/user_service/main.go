...
RPOrigins:      []string{"https://homeserver:10001", "https://pwa.mariusgoehring.de"},
	...
	handler.HandleFunc("/auth/validatetotp", userDB.ValidateTOTPLogin)
	...
	handler.Handle("/user/disabletwofactor", checkAuthMiddleware(http.HandlerFunc(userDB.Disable2FAForUser)))
	handler.Handle("/user/secondfactor", checkAuthMiddleware(http.HandlerFunc(userDB.Enable2FAForUser)))
